...
public class DocumentPendingMailJob implements Job{
	private static final Log log = LogFactory.getLog(DocumentPendingMailJob.class);	
	
	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		log.info("Entered into DocumentPendingMailJob"+Calendar.getInstance().getTimeInMillis());
		try {
			Properties prop = new Properties();
			InputStream in = DocumentPendingMailJob.class.getClassLoader().getResourceAsStream(CMSConstants.APPLICATION_PROPERTIES);
	        try
	        {
	        	prop.load(in);
	        }
			...
			if(mailIds != null){
				...
				List<GroupTemplate> list= temphandle.getDuplicateCheckList("Phd- Document pending Reminder mails");
				if(list != null && !list.isEmpty()) {
					while (iterator.hasNext()) {
						...
						PhdMailBo mail=	new PhdMailBo(pendingDetails.getId(),pendingDetails.getStudentId(),pendingDetails.getDocumentId(),new Date(),pendingDetails.getAssignDate(),"Document Reminder Mail",pendingDetails.getCreatedBy(),pendingDetails.getCreatedDate());					
						...
						mailto.setSubject("Document pending Reminder mails - Christ University");
			...
			log.error("Error in DocumentPendingMail"+Calendar.getInstance().getTimeInMillis());
			e.printStackTrace();
		}
		log.info("Exit from DocumentPendingMail"+Calendar.getInstance().getTimeInMillis());
	}
}
