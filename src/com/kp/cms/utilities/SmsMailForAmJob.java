...
public class SmsMailForAmJob implements Job{
	private static final Log log = LogFactory.getLog(SmsMailForAmJob.class);	
	
	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		log.info("Entered into SmsMailForAmJob"+Calendar.getInstance().getTimeInMillis());
		try {
			...
			List<Object[]> mailIds = AdmissionFormTransactionImpl.getInstance().SendMailForAm(year);
			...
			if(mailIds != null){
				...
				if(mailList != null && !mailList.isEmpty() && sendMail!=null && sendMail.equalsIgnoreCase("true")) {
					while (iter.hasNext()) {
						...
						mailto.setSubject("Reminder mails For Tomorrow Exam - Christ University");
			...
			log.error("Error in SmsMailForAmJob"+Calendar.getInstance().getTimeInMillis());
			e.printStackTrace();
		}
		log.info("Exit from SmsMailForAmJob"+Calendar.getInstance().getTimeInMillis());
	}
}
