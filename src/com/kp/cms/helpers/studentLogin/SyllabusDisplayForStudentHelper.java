...
public class SyllabusDisplayForStudentHelper {
	private static final Log log = LogFactory.getLog(SyllabusDisplayForStudentHelper.class);	
	public static volatile SyllabusDisplayForStudentHelper syllabusDisplayForStudentHelper = null;
	
	public static SyllabusDisplayForStudentHelper getInstance() {
		if (syllabusDisplayForStudentHelper == null) {
			syllabusDisplayForStudentHelper = new SyllabusDisplayForStudentHelper();
		}
		return syllabusDisplayForStudentHelper;
	}

	public List<SyllabusDisplayForStudentTo> convertBotoTo(List<Object[]> boList,Student studentBo,SyllabusDisplayForStudentForm form )throws Exception{
		List<SyllabusDisplayForStudentTo> toList=new ArrayList<SyllabusDisplayForStudentTo>();
	...
	if(boList!=null && !boList.isEmpty()){
		...
		while(itr.hasNext()){
			...
			SyllabusDisplayForStudentTo to=new SyllabusDisplayForStudentTo();
			...
			// */ getting the  number of chances to left
			 ...
			 if(chance<3){
				 year=String.valueOf(admitedYear);
					...
					List<Object[]> admList=SyllabusDisplayForStudentHandler.getInstance().getCurrentStudyingBatchjoiningYear(form);
					...
					while(itr1.hasNext()){
						...
						String subjectName1=SyllabusDisplayForStudentHandler.getInstance().getcurrentStudyingBatchSubjectName(curSchemeDurationId,form);
