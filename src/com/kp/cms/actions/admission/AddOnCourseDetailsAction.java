...
import com.kp.cms.handlers.admission.AddOnCourseDetailsHandler;
 ...
 *
 */
public class AddOnCourseDetailsAction extends BaseDispatchAction {
	
	private static final Log log = LogFactory.getLog(AddOnCourseDetailsAction.class);
	...
	public ActionForward initAddOnCourse(ActionMapping mapping,
		...
		return mapping.findForward("addOnCourseDetailsInit");
	}
	...
	public ActionForward addAddOnCourse(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if (errors.isEmpty()) {
			try {
				...
				subjectGroupDetailsForm.setSpecializationMap1(null);
				boolean isAdded=AddOnCourseDetailsHandler.getInstance().addSubjectGroups(subjectGroupDetailsForm);
				...
				if (subjectGroupDetailsForm.getClassSchemewiseId() != null
					...
					Map<Integer,String>	subjectGroupMap=AddOnCourseDetailsHandler.getInstance().getSubjectGroupsId(subjectGroupDetailsForm);
					subjectGroupDetailsForm.setCertificateMap(subjectGroupMap);
	...
	private void setDatatoForm(SubjectGroupDetailsForm subjectGroupDetailsForm) throws Exception {
		...
		List<SubjectGroupDetailsTo> subjectGroupTo = AddOnCourseDetailsHandler.getInstance()
	...
	public ActionForward getStudentDetails(ActionMapping mapping,
		...
		try {
			...
			AddOnCourseDetailsHandler subjectHandler = AddOnCourseDetailsHandler
					.getInstance();
			List<StudentCertificateCourseTO> subjectList = subjectHandler.getStudentDetails(subjectGroupDetailsForm);
			...
			subjectGroupDetailsForm.setFlag(flag);
			subjectGroupDetailsForm.setAddOnCourseList(subjectList);
			//Used to get subjectGroupMap for based on the class
			if (subjectGroupDetailsForm.getClassSchemewiseId() != null
							&& subjectGroupDetailsForm.getClassSchemewiseId().length() != 0) {
				Map<Integer,String>	subjectGroupMap= subjectHandler.getSubjectGroupsId(subjectGroupDetailsForm);
				subjectGroupDetailsForm.setCertificateMap(subjectGroupMap);
			}
			setClassMapToRequest(request,subjectGroupDetailsForm);
			setDatatoForm1(subjectGroupDetailsForm);
	...
	private void setDatatoForm1(SubjectGroupDetailsForm subjectGroupDetailsForm) throws Exception {
		...
		List<CertificateCourseTO> subjectGroupNameTo = AddOnCourseDetailsHandler.getInstance()
				.getSubjectGroupNames(subjectGroupDetailsForm);
		//setting list to form.
		subjectGroupDetailsForm.setCertificateCourseListTo(subjectGroupNameTo);
		log.info("end of setDatatoForm method in SubjectGroupDetailsAction class.");	
	 ...
	 * @throws Exception
	 */
	public ActionForward editAddOnCourse(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		try {
			...
			AddOnCourseDetailsHandler subjectGroupDetailsHandler = AddOnCourseDetailsHandler.getInstance();
			List<StudentCertificateCourseTO> subjectGroup=subjectGroupDetailsHandler.getEditSubjectGroups(subjectGroupDetailsForm);
			//specializationMap
			...
			subjectGroupDetailsForm.setAddOnCourseList(subjectGroup);
			subjectGroupDetailsForm.setDisplayMessage(null);
			subjectGroupDetailsForm.setDisplayMsg(null);
			subjectGroupDetailsForm.setSpecializationMessage(null);
	 ...
	 */
	@SuppressWarnings("deprecation")
	public ActionForward updateAddOnCourse(ActionMapping mapping, ActionForm form,
		...
		if (errors.isEmpty()) {
			try {
				...
				subjectGroupDetailsForm.setSpecializationMessage(null);
				subjectGroupDetailsForm.setSpecializationMap1(null);
				boolean isUpdated=AddOnCourseDetailsHandler.getInstance().updateSubjectGroups(subjectGroupDetailsForm);
				...
				subjectGroupDetailsForm.setSubjectList(null);
				List<StudentCertificateCourseTO> subjectList = AddOnCourseDetailsHandler.getInstance().getStudentDetails(subjectGroupDetailsForm);
				subjectGroupDetailsForm.setAddOnCourseList(subjectList);
	...
	private void setSpecializationDataToForm(SubjectGroupDetailsForm subjectGroupDetailsForm) throws Exception{
		List<ExamSpecializationTO> specializationTOs = AddOnCourseDetailsHandler.getInstance().getSpecializationData(subjectGroupDetailsForm);
