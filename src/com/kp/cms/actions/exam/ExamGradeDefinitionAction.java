...
import com.kp.cms.handlers.exam.ExamGradeDefinitionHandler;
import com.kp.cms.to.exam.ExamGradeDefinitionTO;

public class ExamGradeDefinitionAction extends BaseDispatchAction {
	ActionErrors errors = new ActionErrors();
	ActionMessages messages = new ActionMessages();
	ExamGradeDefinitionHandler handler = new ExamGradeDefinitionHandler();

	public ActionForward initGradeDefinition(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ExamGradeDefinitionForm objform = (ExamGradeDefinitionForm) form;
		objform.setListExamCourseGroup(handler.init());
		objform.setListCourseName(handler.getListExamCourseUtil());
		return mapping.findForward(CMSConstants.EXAM_GRADE_DEFINITION);
	}
		...
		try {
			if (errors.isEmpty()) {
				if ((Double.valueOf(objform.getStartPercentage().trim())
					...
					objform.setListCourseName(handler.getListExamCourseUtil());
					return mapping
							.findForward(CMSConstants.EXAM_GRADE_DEFINITION);
				}
			} else {

				objform.setListGradeDefinition(handler.select(listCourses));
				objform.setListCourseName(handler
						.getListExamCourse(listCourses));
				return mapping
						.findForward(CMSConstants.EXAM_GRADE_DEFINITION_ADD);
			...
			return mapping.findForward(CMSConstants.EXAM_GRADE_DEFINITION_ADD);
		} catch (ReActivateException e1) {
			errors
					.add("error", new ActionError(
							"knowledgepro.exam.gradeDefinition.reactivate", e1
	...
	private void setRequestToList(ExamGradeDefinitionForm objform,
		...
		objform.setListGradeDefinition(handler.select(listcourseId));
	}
	...
	public ActionForward editGCD(ActionMapping mapping, ActionForm form,
		...
		ArrayList<ExamGradeDefinitionTO> list = handler.select(listcourseId);
		objform.setListGradeDefinition(list);
