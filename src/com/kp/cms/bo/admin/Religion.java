package com.kp.cms.bo.admin;

// Generated Jan 22, 2009 3:35:47 PM by Hibernate Tools 3.2.0.b9

import java.util.Date;
import java.util.HashSet;
import java.util.Set;

/**
 * Religion generated by hbm2java
 */
public class Religion implements java.io.Serializable {

	private int id;
	private String createdBy;;
	private String modifiedBy;
	private String name;
	private Date createdDate;
	private Date lastModifiedDate;
	private Set<WeightageDefinition> weightageDefinitions = new HashSet<WeightageDefinition>(
			0);
	private Set<ReligionSection> religionSections = new HashSet<ReligionSection>(
			0);
	private Set<PersonalData> personalDatas = new HashSet<PersonalData>(0);
	private Boolean isActive;
	public Religion() {
	}

	public Religion(int id) {
		this.id = id;
	}

	public Religion(int id, String createdBy,
			String modifiedBy, String name, Date createdDate,
			Date lastModifiedDate,
			Set<WeightageDefinition> weightageDefinitions,
			Set<ReligionSection> religionSections,
			Set<PersonalData> personalDatas) {
		this.id = id;
		this.createdBy = createdBy;
		this.modifiedBy = modifiedBy;
		this.name = name;
		this.createdDate = createdDate;
		this.lastModifiedDate = lastModifiedDate;
		this.weightageDefinitions = weightageDefinitions;
		this.religionSections = religionSections;
		this.personalDatas = personalDatas;
	}

	public int getId() {
		return this.id;
	}

	public void setId(int id) {
		this.id = id;
	}

	public String getCreatedBy() {
		return this.createdBy;
	}

	public void setCreatedBy(String createdBy)  {
		this.createdBy = createdBy;
	}

	public String getModifiedBy()  {
		return this.modifiedBy;
	}

	public void setModifiedBy(String modifiedBy) {
		this.modifiedBy = modifiedBy;
	}

	public String getName() {
		return this.name;
	}

	public void setName(String name) {
		this.name = name;
	}

	public Date getCreatedDate() {
		return this.createdDate;
	}
	
	public Boolean getIsActive() {
		return isActive;
	}

	public void setIsActive(Boolean isActive) {
		this.isActive = isActive;
	}

	public void setCreatedDate(Date createdDate) {
		this.createdDate = createdDate;
	}

	public Date getLastModifiedDate() {
		return this.lastModifiedDate;
	}

	public void setLastModifiedDate(Date lastModifiedDate) {
		this.lastModifiedDate = lastModifiedDate;
	}

	public Set<WeightageDefinition> getWeightageDefinitions() {
		return this.weightageDefinitions;
	}

	public void setWeightageDefinitions(
			Set<WeightageDefinition> weightageDefinitions) {
		this.weightageDefinitions = weightageDefinitions;
	}

	public Set<ReligionSection> getReligionSections() {
		return this.religionSections;
	}

	public void setReligionSections(Set<ReligionSection> religionSections) {
		this.religionSections = religionSections;
	}

	public Set<PersonalData> getPersonalDatas() {
		return this.personalDatas;
	}

	public void setPersonalDatas(Set<PersonalData> personalDatas) {
		this.personalDatas = personalDatas;
	}

}
