package com.kp.cms.bo.admission;

import java.io.Serializable;

public class AdmissionSubject implements Serializable{


	private int id;
	private String classes;
	private String subject1;
	private String subcode1;
	private String subject2;
	private String subcode2;
	private String subject3;
	private String subcode3;
	private String subject4;
	private String subcode4;
	private String subject5;
	private String subcode5;
	private String academicYear;
	
	public AdmissionSubject(){
		
	}
	public AdmissionSubject(int id, String classes, String subject1,
			String subcode1, String subject2, String subcode2, String subject3,
			String subcode3, String subject4, String subcode4, String subject5,
			String subcode5, String academicYear) {
		super();
		this.id = id;
		this.classes = classes;
		this.subject1 = subject1;
		this.subcode1 = subcode1;
		this.subject2 = subject2;
		this.subcode2 = subcode2;
		this.subject3 = subject3;
		this.subcode3 = subcode3;
		this.subject4 = subject4;
		this.subcode4 = subcode4;
		this.subject5 = subject5;
		this.subcode5 = subcode5;
		this.academicYear = academicYear;
	}
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getClasses() {
		return classes;
	}
	public void setClasses(String classes) {
		this.classes = classes;
	}
	public String getSubject1() {
		return subject1;
	}
	public void setSubject1(String subject1) {
		this.subject1 = subject1;
	}
	public String getSubcode1() {
		return subcode1;
	}
	public void setSubcode1(String subcode1) {
		this.subcode1 = subcode1;
	}
	public String getSubject2() {
		return subject2;
	}
	public void setSubject2(String subject2) {
		this.subject2 = subject2;
	}
	public String getSubcode2() {
		return subcode2;
	}
	public void setSubcode2(String subcode2) {
		this.subcode2 = subcode2;
	}
	public String getSubject3() {
		return subject3;
	}
	public void setSubject3(String subject3) {
		this.subject3 = subject3;
	}
	public String getSubcode3() {
		return subcode3;
	}
	public void setSubcode3(String subcode3) {
		this.subcode3 = subcode3;
	}
	public String getSubject4() {
		return subject4;
	}
	public void setSubject4(String subject4) {
		this.subject4 = subject4;
	}
	public String getSubcode4() {
		return subcode4;
	}
	public void setSubcode4(String subcode4) {
		this.subcode4 = subcode4;
	}
	public String getSubject5() {
		return subject5;
	}
	public void setSubject5(String subject5) {
		this.subject5 = subject5;
	}
	public String getSubcode5() {
		return subcode5;
	}
	public void setSubcode5(String subcode5) {
		this.subcode5 = subcode5;
	}
	public String getAcademicYear() {
		return academicYear;
	}
	public void setAcademicYear(String academicYear) {
		this.academicYear = academicYear;
	}
	
}
