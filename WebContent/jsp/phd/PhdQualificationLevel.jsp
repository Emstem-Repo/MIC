...
<script type="text/javascript">
	...
	function resetValue() {
	 ...
	 function editQualificationLevel(id){
		document.location.href = "phdQualification.do?method=editQualificationLevel&id=" + id;
		}
	function reActivate() {
		
		//var id = document.getElementById("duplId").value;
		document.location.href = "phdQualification.do?method=activateQualificationLevel";
	}		
	function deleteQualificationLevel(id) {
		deleteConfirm = confirm("Are you sure you want to delete this entry?")
		if (deleteConfirm) {
			document.location.href = "phdQualification.do?method=deleteQualificationLevel&id="+ id ;
		}
	}
	...
	function cancelAction1(){
		resetFieldAndErrMsgs();
		document.location.href = "phdQualification.do?method=initQualificationLevel";
	}
</script>
<html:form action="/phdQualification">
	<c:choose>
	...
	<html:hidden property="formName" value="phdQualificationLevelForm" />
	<html:hidden property="duplId" name = "phdQualificationLevelForm" value="duplId" />
	...
	<table width="100%" border="0">
	  <tr>
	    <td><span class="heading"><span class="Bredcrumbs"><bean:message key="knowledgepro.phd"/> &gt;&gt; <bean:message key="knowledgepro.employee.qualification.level"/> &gt;&gt;</span></span></td>
	  </tr>
	  <tr>
	          ...
	          <tr>
	            ...
	            <td valign="top"><table width="100%" cellspacing="1" cellpadding="2">
	              <tr >
					...
					<html:text property="name" styleId="qualificationId" name="phdQualificationLevelForm" maxlength="50"></html:text>
					</td>
	                ...
	                <td width="33%" class="row-even"><span class="star">
	                  <html:text property="displayOrder" styleId="displayOrder" maxlength="50"  size="8" onkeypress="return isNumberKey(event)"/>
	                </span></td>
	              </tr>
	              <tr>
					...
					<html:radio property="fixedDisplay" name="phdQualificationLevelForm" value="true" styleId="fixed">Yes</html:radio>
					<html:radio property="fixedDisplay"  name="phdQualificationLevelForm" value="false" styleId="fixed">No</html:radio>
					</td>
	              </tr>
	            </table></td>
		...
		<tr>
			<td width="45%" height="35">
					<div align="right"><c:choose>
						<c:when test="${QualificationLevelEntryOperation != null && QualificationLevelEntryOperation == 'edit'}">
			...
			<td width="2%">
						<c:choose>
							<c:when test="${QualificationLevelEntryOperation != null && QualificationLevelEntryOperation == 'edit'}">
								<html:button property="" styleClass="formbutton" value="Reset" onclick="resetValue()"></html:button> 
						</c:when>
						<c:otherwise>
							<html:button property="" styleClass="formbutton" value="Reset" onclick="resetValue()"></html:button>
						</c:otherwise>
				...
				<logic:notEmpty name="QualificationLevelList">
					<logic:iterate id="QualificationLevelList" name ="QualificationLevelList" indexId="count">
						<c:choose>
			                ...
			                <td width="25%" height="25" align="center"><bean:write name = "QualificationLevelList" property="name"/></td>
			                <td width="25%" align="center"><bean:write name = "QualificationLevelList" property="displayOrder"/></td>
			                 <td width="25%" align="center"><bean:write name = "QualificationLevelList" property="fixedDisplay"/></td>
