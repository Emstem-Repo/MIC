...
<script>
	...
	function viewOldMarks(subjectId, examMasterId) {
		...
		answerScriptType=document.getElementById("answerScriptType").value;
		evaluatorType=document.getElementById("evaluatorType").value;
		secured=document.getElementById("secured").value;
		markType=document.getElementById("markType").value;
		var url="ExamStudentMarksCorrection.do?method=viewOldMarks&examNameId="+examNameId+"&studentId="+studentId+"&subjectId="+subjectId+"&answerScriptTypeId="+answerScriptType+"&evaluatorTypeId="+evaluatorType+"&secured="+secured+"&markType="+markType + "&examMasterId=" + examMasterId;
		window.open(url,'Student Marks Correction');
	
	
	}
	function viewOverAllOldMarks(subjectId, examMasterId) {
		examNameId=document.getElementById("examNameId").value;
		studentId=document.getElementById("studentId").value;
		markType=document.getElementById("markType").value;
		//scheme=document.getElementById("schemeId").value;
		var url="ExamStudentMarksCorrection.do?method=viewOldMarksOverall&examNameId="+examNameId+"&studentId="+studentId+"&subjectId="+subjectId+"&markType="+markType + "&examMasterId=" + examMasterId;
		window.open(url,'Student Marks Correction');
	}
	function clearList(){
		document.getElementById("method").value="clearListOnSecuredChange";
		document.ExamStudentMarksCorrectionForm.submit();
	}
			
</script>

<html:form action="/ExamStudentMarksCorrection.do" method="POST"
	enctype="multipart/form-data" focus="regNo">
							...
							<table width="100%" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td width="23%" height="25" class="row-even"><html:hidden
										property="courseId" name="ExamStudentMarksCorrectionForm" />
									...
									<td width="26%" class="row-even"><html:hidden
										property="schemeId" name="ExamStudentMarksCorrectionForm" styleId="schemeId" />
								...
								<tr>
									...
									<td height="25" class="row-even"><bean:write
										...
										<c:if test="${ExamStudentMarksCorrectionForm.markType != 'Internal overAll' && ExamStudentMarksCorrectionForm.markType != 'Regular overAll' }">
											<logic:notEmpty name="evaluatorMap">
												<td height="25" class="row-odd">
								...
								<c:if test="${ExamStudentMarksCorrectionForm.markType != 'Internal overAll' && ExamStudentMarksCorrectionForm.markType != 'Regular overAll' }">
									...
									<td height="25" class="row-odd">
										...
										<td height="25" class="row-even"><html:select
											...
											<logic:notEmpty name="answerScriptTypeMap">
												<html:optionsCollection name="answerScriptTypeMap" label="value" value="key" />
											</logic:notEmpty>
										</html:select></td>
									</tr>
								</c:if>
				...
				<tr>
					...
					<table width="100%" border="0" align="center" cellpadding="0"
						...
						<tr>
							...
							<c:if test="${ExamStudentMarksCorrectionForm.markType != 'Internal overAll' && ExamStudentMarksCorrectionForm.markType != 'Regular overAll' }">
								<tr>
									<td height="25" colspan="6" class="row-odd">
										<input type="hidden" name="sec" id="test" value='<nested:write property="dupsecured" name="ExamStudentMarksCorrectionForm"/>'/>
										<nested:checkbox property="secured" styleId="secured" onchange="clearList()">Secured</nested:checkbox>
											<script type="text/javascript" language="javascript">
												var sec= document.getElementById("test").value;
												if(sec == "on") {
													document.getElementById("secured").checked = true;
												}
											</script>
									</td>
								</tr>
									...
									<table width="100%" cellspacing="1" cellpadding="2">
										...
										<nested:iterate property="singleStuFor_AllSubjects"
											...
											<tr>
												...
												<c:choose>
												<c:when test="${ExamStudentMarksCorrectionForm.markType != 'Internal overAll' && ExamStudentMarksCorrectionForm.markType != 'Regular overAll' }">
													<td width="16%" align="center" class="row-even"><nested:equal
														...
														property="examMasterId" />')" style="CURSOR:hand">View
														Old Marks</u>
													</nested:equal></td>
												</c:when>
												<c:otherwise>
													<td width="16%" align="center" class="row-even"><nested:equal
															...
															<u
																onclick="viewOverAllOldMarks('<nested:write
															property="subjectId" />', '<nested:write
															property="examMasterId" />')" style="CURSOR:hand">View
															Old Marks</u>
														</nested:equal></td>
								...
								<tr>
									...
									<table width="100%" cellspacing="1" cellpadding="2">
										<tr>
											...
											<html:hidden property="answerScriptType"
												styleId="answerScriptType" />
											<html:hidden property="evaluatorType" styleId="evaluatorType" />
