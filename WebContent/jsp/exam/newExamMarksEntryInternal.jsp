...
<SCRIPT>
	function cancelAction() {
		document.location.href = "marksEntry.do?method=initExamMarksEntryForInternal";
	}
	...
	function getCourse(examName) {
		...
		if(roleIdForTeacher.indexOf(roleId)!=-1){
			getCourseByExamNameByTeacher("schemeMap", examName, year, "course", updateToCourse);
		}			
		else{
			getCourseByExamName1("schemeMap", examName, year, "course", updateToCourse);	
		}
	}
	...
	function getScheme(courseId) {
		...
		var roleIdForTeacher = document.getElementById("roleIdForTeacher").value;	
		if(roleIdForTeacher.indexOf(roleId)!=-1)
			getSchemeNoByExamIdCourseIdByTeacher("schemeMap", examId, courseId, "scheme",
					updateToScheme);
		else
	...
	function getSectionByScheme(schemeId) {
		...
		var roleIdForTeacher = document.getElementById("roleIdForTeacher").value;	
		if(roleIdForTeacher.indexOf(roleId)!=-1)
			getSectionByExamIdCourseIdSchemeIdByTeacher("sectionMap", courseId, schemeId,examId,
	...
	function getSCodeName(sCName) {
		...
		var roleIdForTeacher = document.getElementById("roleIdForTeacher").value;	
		if(roleIdForTeacher.indexOf(roleId)!=-1)
			getSubjectsCodeNameByCourseSchemeExamIdByTeacher("schemeMap", sCName, courseId, "subject",
					updateToSubject, schemeId,examId);
		else			
