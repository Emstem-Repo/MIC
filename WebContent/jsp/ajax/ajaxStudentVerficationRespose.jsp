...
<response>
	<c:if test="${msg==null}">
		<logic:iterate id="StudentMarksTo" name="list">
			...
			<tMarks><bean:write name="StudentMarksTo" property="theoryMarks"/></tMarks>
			<pMarks><bean:write name="StudentMarksTo" property="practicalMarks"/></pMarks>
			<mistake><bean:write name="StudentMarksTo" property="mistake"/></mistake>
			</fields>
		</logic:iterate>
		<c:if test="${regMsg!=null}">
			<regMsg><bean:write name="regMsg"/> </regMsg>
		</c:if>
	</c:if>
	<c:if test="${msg!=null}">
