...
/*			if (document.getElementById("student_" + field.id).disabled == false) {
				document.getElementById("student_" + field.id).value = field.value;
			}*/
			...
			for ( var i = 0; i < document.forms[0].elements.length; i++) {
				...
				var e = document.forms[0].elements[i];
					...
					if ((e.type == 'text') && (e.disabled == false)) {
					
						if (e.id == "student_"+field.id ) {
							e.value = field.value;					
						}
				}
			}
	...
	function setFieldValues(field) {
		var myString = field.id;
		var mySplitResult = myString.split("student_");		
		field.value =	document.getElementById(mySplitResult[1]).value;
	}

	
	function cancelAction() {
		document.location.href = "MonthlyAttendanceEntry.do?method=initMonthlyAttendanceEntry";
	}

	function validNumber(field) {
		var mytool_array = field.id.split("student_");
		var errorMessage = "";

		if (isNaN(field.value)) {
			field.value = "";
		} else {
			if (document.getElementById(mytool_array[1]).value == ""
					&& field.value != "") {
				errorMessage = errorMessage
						+ "Enter value for hours held for day  "
						+ (parseInt(mytool_array[1]) + 1);
	...
	function submitMonthlyAttendanceEntry() {
		if(validateMonthlyAttendance()) {
			document.getElementById("method").value = "persistMonthlyAttendanceList";
	...
	function validateMonthlyAttendance() {
				...
				if (splitId == "student") {
					...
					if (dayValue == "" && fieldValue != "") {
						...
						if (parseInt(dayValue) < parseInt(fieldValue)) {
							errorMessage = errorMessage
									+ "Hours attended cannot be greater than Hours held for day  "
									+ (parseInt(dayId) + 1);
							document.getElementById("err").innerHTML = errorMessage;
							return false;
						}
		...
		if (!isRecordedAdded) {
			errorMessage = errorMessage
					+ "Please add Hours Held and Hours Attended values.  ";
			document.getElementById("err").innerHTML = errorMessage;
			return false;
		}
		document.getElementById("err").innerHTML = errorMessage;
	...
	<table width="99%" border="0">
		...
		<tr>
			<td><span class="heading"><bean:message
				...
				key="knowledgepro.attendanceentry.monthlyattendance" /></span></span></td>
		</tr>
		<tr>
			<td>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td background="images/Tcenter.gif" class="heading_white"><bean:message
						key="knowledgepro.attendanceentry.monthlyattendance" /></td>
						...
						<tr>
							...
							<table width="100%" cellspacing="1" cellpadding="2">
								...
								<tr>
									...
									<td class="row-even"><bean:write
										name="monthlyAttendanceEntryForm" property="batchName" /></td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<table width="99%" border="0" align="center" cellpadding="0"
								...
								<tr>
									<td width="5" background="images/left.gif"/>
									</td>	
									...
									<table width="100%" cellspacing="1" cellpadding="2"><tr>
											...
											<nested:iterate name="monthlyAttendanceEntryForm"
												...
												<bean:define id="tabOrder" name="id" property="tabOrder"/>		
												
												<%String tab = String.valueOf(tabOrder); %>										
												<logic:empty name="id" property="hoursHeld">

													<nested:text  property="hoursHeld" styleId="<%=style %>" tabindex="<%=tab %>"
														styleClass="TextBox" style="width: 10px"   maxlength="1"
														onblur="setTextValues(this)">
													</nested:text>
												</logic:empty> 
												<logic:notEmpty name="id"
													...
													<nested:text  property="hoursHeld" styleId="<%=style %>" tabindex="<%=tab %>"
														styleClass="TextBox" style="width: 10px"  maxlength="1"
														onblur="setTextValues(this)" disabled="true"></nested:text>
												</logic:notEmpty>												
											</div>
												</td>
											</nested:iterate>
										...
										<tr>
											<td width="15" class="row-odd" align="center"><c:choose>
											...
											<nested:iterate name="monthlyAttendanceEntryForm"
												...
												<td width="1" height="25" class="row-odd" align="center">
												...
												<c:when test="${temp == 0}">
													...
													<tr>
														...
														<nested:iterate  property="hoursHeldTOList" 
															indexId="count1" type="com.kp.cms.to.attendance.HourseHeldTO">
															<%String abc = "student_" + count1;%>															
															<bean:define id="tabOrder" name="hoursHeldTOList" property="tabIndex"></bean:define>
															<%String tab = String.valueOf(tabOrder); %>	
															<td width="1%" height="25" class="row-even">
															<div align="center"><span class="bodytext">

															<logic:empty name="hoursHeldTOList" property="hourseHeld">

																<nested:text  property="hourseHeld" styleId="<%=abc %>" tabindex="<%=tab %>"
																	styleClass="TextBox" style="width: 10px"  maxlength="1" 
																	onblur="validNumber(this)"> </nested:text>
															</logic:empty> <logic:notEmpty name="hoursHeldTOList"
																property="hourseHeld">
																<nested:text name="hoursHeldTOList" property="hourseHeld" styleId="<%=abc %>" tabindex="<%=tab %>"
																	styleClass="TextBox" style="width: 10px"   maxlength="1" 
																	onblur="validNumber(this)" disabled="true"></nested:text>
													...
													<tr>
														...
														<nested:iterate  property="hoursHeldTOList" 
															...
															<%String abc = "student_" + count1;%>
																...
																<bean:define id="tabOrder" name="hoursHeldTOList" property="tabIndex"/>	
																<%String tab = String.valueOf(tabOrder); %>	
															<div align="center"><span class="bodytext">
															<logic:empty name="hoursHeldTOList" property="hourseHeld">
				...
				<tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td height="35" align="center"><html:button	property="" styleClass="formbutton" onclick="submitMonthlyAttendanceEntry()">
