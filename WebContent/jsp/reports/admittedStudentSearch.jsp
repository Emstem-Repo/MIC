...
<script type="text/javascript">
	...
	function getSemisters(year) {
		...
		getSemistersOnYearAndCourse("semistersMap", courseId, "semister", year,
				updateSemisters);
...
<html:form action="/admittedStudentReport" method="post">
	<html:hidden property="method" styleId="method"
		value="getAdmittedStudentsReport" />
	...
	<table width="98%" border="0" cellpadding="2" cellspacing="1">
		<tr>
			<td class="heading"><bean:message key="knowledgepro.reports" /><span
				class="Bredcrumbs">&gt;&gt; <span class="Bredcrumbs"><bean:message
				key="knowledgepro.report.admittedstudentreport" />&gt;&gt;</span></span></td>
		</tr>
		<tr>
			<td valign="top">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td background="images/Tcenter.gif" class="body"><strong
						...
						key="knowledgepro.report.admittedstudentreport" /></strong></td>
						...
						<tr>
							...
							<table width="100%" cellspacing="1" cellpadding="2">
								<tr>
									...
									<td width="28%" height="25" class="row-even"><html:select
									...
									<td width="26%" height="25" class="row-even"><html:select
										...
										<c:if
											test="${admittedStudentsForm.programTypeId != null && admittedStudentsForm.programTypeId != ''}">
								...
								<tr>
									...
									<td class="row-even"><html:select property="courseId"
										styleId="course" styleClass="combo"
										onchange="getSemisters(this.value)">
										<html:option value="">
											<bean:message key="knowledgepro.admin.select" />
										</html:option>
										<c:if
											test="${admittedStudentsForm.programId != null && admittedStudentsForm.programId != ''}">
											<c:set var="courseMap"
									...
									<td height="25" colspan="4" class="row-even"><input
										...
										value="<bean:write name="admittedStudentsForm" property="academicYear"/>" />
									<html:select property="academicYear" styleId="academicYear"
										styleClass="combo" onchange="getSemisters(this.value)">
								...
								<tr>
									<td height="25" class="row-odd">
									<div align="right"><bean:message
										key="curriculumSchemeForm.schemeId" />:</div>
									</td>
									<td height="25" class="row-even"><html:select
										property="semister" styleId="semister" styleClass="combo">
										<html:option value="">- Select -</html:option>
										<c:if
											test="${admittedStudentsForm.academicYear != null && admittedStudentsForm.academicYear != ''}">
											<c:set var="semistersMap"
												value="${baseActionForm.collectionMap['semistersMap']}" />
											<c:if test="${semistersMap != null}">
												<html:optionsCollection name="semistersMap" label="value"
													value="key" />
											</c:if>
										</c:if>
									</html:select></td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td width="53"><html:button property=""
								styleClass="formbutton" value="Reset" onclick="reset()"></html:button></td>
						</tr>
					</table>
					</td>
					<td valign="top" background="images/Tright_3_3.gif" class="news"></td>
