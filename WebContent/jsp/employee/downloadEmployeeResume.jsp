...
function getDetails() {
	document.getElementById("method").value="displayEmployeeDetails";
}

function viewResume(id) {
	document.location.href = "DownLoadEmployeeResume.do?method=getResumeDetails&employeeId="+id;
}

function sendMail(){
	document.getElementById("method").value="initSendMail";
}
...
var print = "<c:out value='${downloadEmployeeResumeForm.printPage}'/>";
	if(print.length != 0 && print == "true"){
		var url = "DownLoadEmployeeResume.do?method=printResume";
		myRef = window
				.open(url, "ViewResume",
						"left=20,top=20,width=900,height=900,toolbar=1,resizable=0,scrollbars=1");	
		}

	function shows(obj,count){
		var style = document.createElement('style');
		style.type = 'text/css';
		style.innerHTML = '.cssClass { border-right: 1px solid #000000; position: absolute;width: 250px; z-index: 1; background-color: #FFFFE0 ; border-style: solid; border-width: 1px; display:none; }';
		document.getElementsByTagName('head')[0].appendChild(style);

		document.getElementById("messageBox_"+count).className = 'cssClass';
		var names = document.getElementById("forward_"+count).value;
		if(names!=""){
		document.getElementById("messageBox_"+count).style.display="block";
		document.getElementById("contents_"+count).innerHTML=names;
		}
	}
...
<html:hidden property="method" styleId="method" value="displayEmployeeDetails"/>	
...
<c:choose>
		        	...
		        	<label>
						<html:select property="departmentId" styleId="departmentId" styleClass="combo">
							<html:option value=""><bean:message key="knowledgepro.admin.select" /></html:option>
								<logic:notEmpty name="downloadEmployeeResumeForm" property="departmentList">
		        ...
		        <td height="25" class="row-even">
		        	<label>
						<html:select property="empSubjectId" styleId="empSubjectId" styleClass="combo">
							<html:option value=""><bean:message key="knowledgepro.admin.select" /></html:option>
								<logic:notEmpty name="downloadEmployeeResumeForm" property="empSubjects">
           ...
           <tr>
              <td height="25" class="row-odd"><div align="right" id="regLabel">Qualification level:</div></td>
		        <td height="25" class="row-even" align="left"> <label>
					<html:select property="qualificationId" styleId="qualificationId" styleClass="combo">
							<html:option value=""><bean:message key="knowledgepro.admin.select" /></html:option>
								<logic:notEmpty name="downloadEmployeeResumeForm" property="qualificationList">
               ...
               <td height="25" class="row-odd">
				...
				<td class="row-even">
						<html:text name="downloadEmployeeResumeForm" property="startDate" styleId="startDate" size="11" maxlength="11" />
              ...
              <tr>
				...
				<td class="row-even">
					<html:text name="downloadEmployeeResumeForm" property="name" styleId="name" />
				</td>
              </tr>
              <tr>
				<td height="25" class="row-odd">
				...
				<td class="row-even">
					<html:text name="downloadEmployeeResumeForm" property="applnNo" styleId="applnNo" />
				</td>
				<td class="row-odd">
					...
					<table width="100%" border="0" align="center" cellpadding="0"
						...
						<tr>
                            ...
                            <c:otherwise>
								...
								<logic:notEmpty property="downloadEmployeeResumeTOs" name="downloadEmployeeResumeForm">
									
									<nested:iterate id="to" property="downloadEmployeeResumeTOs" name="downloadEmployeeResumeForm" indexId="count">
										<c:choose>
										...
										<% String names = "forward_"+count;
											String messageBox = "messageBox_"+count;
											String contents = "contents_"+count;
										%>
										<html:hidden property="forwardedUsers" name="to" styleId='<%=names %>'/>
										...
										<td align="center" width="18%" height="25">
										<c:choose>
                            <c:when test="${downloadEmployeeResumeForm.isCjc }">
                            ...
                            <c:otherwise>
                                <nested:write property="departmentName" />
										...
										<td align="center" width="11%" height="25" onmouseover="shows(this,<c:out value='${count}'/>)" onmouseout="hides(<c:out value='${count}'/>)">
										<nested:write property="currentStatus"  />
										<div id='<%=messageBox %>'>
					...
					<td valign="top" class="news">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<div align="right">
								<logic:notEmpty property="downloadEmployeeResumeTOs" name="downloadEmployeeResumeForm">
									<html:submit property="" styleClass="formbutton" onclick="sendMail()">Forward</html:submit>
								</logic:notEmpty>
								</div>
								</td>
								<td width="2%"></td>
