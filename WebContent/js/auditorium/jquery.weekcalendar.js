...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      goToDate : function() {
         ...
         this._clearCalendar();
         this._loadCalEvents(new Date(date1));
      },
      ...
      _renderCalendar : function() {
         ...
         if (options.buttons) {
            calendarNavHtml = "<div class=\"wc-nav\">\
                    <button class=\"CSSButtonToday\">" + options.buttonText.today + "</button>\
                    <button class=\"CSSButtonPrev\">" + options.buttonText.lastWeek + "</button>\
                    <button class=\"CSSButtonNext\">" + options.buttonText.nextWeek + "</button>\
                    </div>";

            $(calendarNavHtml).appendTo($calendarContainer);

            $calendarContainer.find(".wc-nav .CSSButtonToday").click(function() {
               self.element.weekCalendar("today");
               return false;
            });

            $calendarContainer.find(".wc-nav .CSSButtonPrev").click(function() {
               self.element.weekCalendar("prevWeek");
               return false;
            });

            $calendarContainer.find(".wc-nav .CSSButtonNext").click(function() {
      ...
      _dateFirstDayOfWeek : function(date) {
         ...
         if(count==1) {
        	 //var millisToSubtract =  86400000;
        	 return new Date(midnightCurrentDate.getTime());}
         else if(count>1){
